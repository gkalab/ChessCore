...
static void testEpdPerft(const string &epd) {
    EpdFile epdFile;
    EXPECT_TRUE(epdFile.readFromString(epd));
    for (size_t i = 0; i < epdFile.numEpds(); i++) {
        const Epd *epd = epdFile.epd(i);
        EXPECT_NE((Epd *)0, epd);
        for (size_t j = 0; j < epd->numOps(); j++) {
            const EpdOp *op = epd->op(j);
            EXPECT_NE((EpdOp *)0, op);

            unsigned depth = 0;
            if (op->opcode() == "d1" || op->opcode() == "perft1") {
                depth = 1;
            } else if (op->opcode() == "d2" || op->opcode() == "perft2") {
                depth = 2;
            } else if (op->opcode() == "d3" || op->opcode() == "perft3") {
                depth = 3;
            } else if (op->opcode() == "d4" || op->opcode() == "perft4") {
                depth = 4;
            } else if (op->opcode() == "d5" || op->opcode() == "perft5") {
                depth = 5;
            } else if (op->opcode() == "d6" || op->opcode() == "perft6") {
                depth = 6;
            } else if (op->opcode() == "d7" || op->opcode() == "perft7") {
                depth = 7;
            } else if (op->opcode() == "d8" || op->opcode() == "perft8") {
                depth = 8;
            } else if (op->opcode() == "d9" || op->opcode() == "perft9") {
                depth = 3;
            }

            if (depth > 0) {
                cerr << "Running EPD line " << (i + 1) << " of " << epdFile.numEpds() <<
                    ", op " << (j + 1) << " of " << epd->numOps() << " at depth " << depth << endl;
                uint64_t expected = op->operandInteger();
                Position pos = epd->pos();
                EXPECT_EQ(expected, perft(pos, depth));
            } else {
                cerr << "Ignored op " << (j + 1) << " of " << epd->numOps() << " (\"" << op->opcode() <<
                    "\") in EPD line " << (i + 1) << " of " << epdFile.numEpds() << endl;
