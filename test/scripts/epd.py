...
def usage():
    ...
    print "    --timelimit=time        The time to analyze each EPD entry"
...
def main():
    ...
    common.parser().add_option("--timelimit", dest="timelimit", default = "10s", help = "Time limit per move")
    ...
    if logcomms:
        cmdline += " --logcomms"
    cmdline += " -c {0} -l {1} -e {2} -t {3} processepd {4}".format(configfile, logfile, epdfile, timelimit, engine);
